...
def train(tokenizer, train_dataset, dev_dataset, args, idx2label, data_collator) -> BioGptModel:
    ...
    ppl_before_training = trainer.compute_perplexity()
    logger.info(f"Perplexity on dev set before training: {ppl_before_training}")

    train_metrics = trainer.train()

    ppl_after_training = trainer.compute_perplexity()
    logger.info(f"Perplexity on dev set after training: {ppl_after_training}")
